...
function display(thisuser){
  ".swal2-popup {background-color: rgb(56, 58, 0);}"
fetch("/JS/leaderboard.json")
  .then(function (response) {
    return response.json();
  })
      ...
      if(usersread == thisuser){
        ...
        for(let i = 0; i < person.levels.length; i++){
         /*
          ...
          compl+= '<p1 class = "playerlevelEntry">'+thisPersonsLevels[i].name+' (#'+thisPersonsLevels[i].pos+')</p1><br>';
        }
        ...
        if(person.progs[0] != "none" && person.progs.length > 0){
          ...
          for(let i = 0; i < thisPersonsProgs.length; i++){
            ...
            playerProgsList+='<p1 class = "playerlevelEntry">'+thisPersonsProgs[i].name + ' ' + thisPersonsProgs[i].percent + '% (#' + thisPersonsProgs[i].pos + ')</p1><br>'; 
          }
        }else{
          playerProgsList = '<p1>none</p1>';
        }
        if(playerProgsList == '<ol>') playerProgsList = '<p>none</p>';
        Swal.fire({
          html:'<p1>Completed levels: </p1>' + compl +'</ol>' + '<p1>Progresses: </p1>' + playerProgsList + '</ol>'
        });
        break;
      }
      usersread++;
